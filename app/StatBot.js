...
// ─── MAIN COMPONENT ─────────────────────────────────────────
export default function StatBot() {
  ...
  // ── Data info string for the prompt ──
  const getDataInfo = () => {
  ...
  const highlight = (txt) => {
    ...
    return txt
      ...
      .replace(
        /\b(library|require|install\.packages|if|else|for|in|function|return|TRUE|FALSE|NULL|NA|NaN|Inf)\b/g,
        '<span style="color:#c084fc">$1</span>'
      )
      .replace(
        /\b(read\.csv|read_sav|as\.data\.frame|cfa|sem|lm|glm|aov|manova|summary|print|cat|sprintf|paste0|paste|c|data\.frame|matrix|factor|ordered|levels|as\.numeric|as\.character|as\.integer|sink|file|write\.csv|cor|mean|sd|var|nrow|ncol|names|str|head|subset|apply|lapply|sapply|aggregate|fitted|residuals|predict|coef|confint|anova|fitMeasures|standardizedEstimates|parameterEstimates|haven|lavaan|psych|mirt|semTools)\b/g,
  ...
  // ─── RENDER ─────────────────────────────────────────────
  return (
    <div style={{ background: "#0a0b0f", color: "#e2e4ea", minHeight: "100vh" }}>
      {/* grain overlay */}
      <div
  style={{
    position: "fixed",
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 760, margin: "0 auto", padding: "40px 20px 70px" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 44 }}>
          <div style={{ fontSize: 40, fontWeight: 700, letterSpacing: -1.5, marginBottom: 6 }}>
            <span style={{ color: "#5eead4" }}>Stat</span>
            <span style={{ color: "#e2e4ea" }}>Bot</span>
          </div>
              ...
              {file.type === "sav" && (
                <div style={{ marginTop: 14, background: "rgba(251,191,36,0.06)", border: "1px solid rgba(251,191,36,0.22)", borderRadius: 10, padding: "14px 16px" }}>
                  ...
                  <div style={{ color: "#9ca3af", fontSize: 12.5, lineHeight: 1.75, marginTop: 7 }}>
                    ...
                    For example: <em style={{ color: "#6b7280" }}>"Items are named SE1 to SE43"</em>
                  </div>
                </div>
              )}
            </div>
        ...
        <StepCard step="3" title="Describe What You Want" done={!!userQuery.trim()}>
          <textarea
            ...
            placeholder={
              file?.type === "sav"
                ? 'Example: "I have a self-efficacy scale with 43 items named SE1 to SE43 on a 5-point Likert scale, distributed across 5 subscales... I want bifactor CFA using WLSMV. Compute all omega indices and save results."'
                : 'Example: "Predict student GPA using study_hours, attendance, and prior_score. Include diagnostics and confidence intervals. Save output to a text file."'
            }
            style={{
          ...
          <div style={{ fontSize: 11.5, color: "#6b7280", marginTop: 8, lineHeight: 1.6 }}>
            ...
            {file?.type === "sav" && <span style={{ color: "#fbbf24" }}> Since you uploaded a .sav, describe your variable names here.</span>}
          </div>
        </StepCard>
